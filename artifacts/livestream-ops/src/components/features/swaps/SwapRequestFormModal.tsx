...
export function SwapRequestFormModal({ 
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.new_host_id) newErrors.new_host_id = 'Please select a replacement host'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await swapRequestService.create({
        ...
        new_host_id: formData.new_host_id,
  ...
  const availableHosts = users.filter(u => u.role === 'staff' && u.department === 'Live Host')
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Replacement Host Selection */}
          <div>
            <label className="text-sm font-medium mb-2 block">
              Proposed Replacement Host <span className="text-red-500">*</span>
            </label>
            <Select value={formData.new_host_id} onValueChange={(value) => setFormData({ ...formData, new_host_id: value })}>
            ...
            <div className="text-xs text-gray-600 mt-1">Note: The selected host must confirm availability separately</div>
          </div>
