...
test('role UX matrix covers every requested operational surface and priority', () => {
  for (const moduleName of modules) {
    for (const role of ['admin', 'leader', 'member'] as const) {
      const surface = getRoleUxSurfaceConfig(user(role), moduleName)
      ...
      assert.equal(surface.priority, roleUxPriorities[moduleName][role])
    }
  }
})
...
test('member surfaces never expose management capabilities', () => {
  const member = user('member')
  for (const moduleName of ['staff', 'brands', 'platforms', 'import'] as const) {
    const surface = getRoleUxSurfaceConfig(member, moduleName)
