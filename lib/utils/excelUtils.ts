...
export function downloadExcelTemplate(): void {
  ...
  const instructions = [
    ...
    { Field: 'Technical', Format: 'Email', Required: 'No', Example: 'technical@example.com' },
    ...
    reader.onload = (e) => {
      try {
        ...
        jsonData.forEach((row: any, index) => {
          ...
          const technicalId = row['Technical'] ? usersMap.get(row['Technical']) : undefined
          ...
          if (row['Technical'] && !technicalId) {
            rowErrors.push({ row: rowNumber, field: 'Technical', message: `Technical '${row['Technical']}' not found` })
          }
