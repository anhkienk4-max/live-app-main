...
export interface ShiftFormWorkspaceProps {
  onBack?: () => void
  shift: Shift | null
...
export function ShiftFormWorkspace({
  onBack,
  shift,
  duplicateFrom,
  brands,
  ...
  React.useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const usesAuthoritativeDefaults = !shift && !duplicateFrom && getAuthMode() === 'supabase'
      if (shift) {
        setFormData({
        title: shift.title || '',
        date: shift.date,
  ...
  return (
    <div className="flex h-full flex-col bg-muted/5 w-full overflow-hidden" data-testid="shift-form-workspace">
      <div className="flex h-full flex-col w-full">
        ...
        <div className="sticky bottom-0 z-10 border-t bg-background px-6 py-4 shadow-sm flex items-center justify-end space-x-2 flex-shrink-0">
