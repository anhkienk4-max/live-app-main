...
export function NotificationCenter() {
  ...
  const getIcon = (type: string, read: boolean) => {
    ...
    const colorClass = read ? "text-muted-foreground" :
      type === 'shift_assigned' ? "text-blue-600" :
      type.includes('import') ? "text-red-600" :
      type.includes('swap') ? "text-amber-600" :
      type.includes('report') ? "text-orange-600" : "text-primary"

    switch (type) {
      case 'shift_assigned': return <Clock className={`${baseClass} ${colorClass}`} />
      case 'import_warning':
      case 'import_failure': return <AlertCircle className={`${baseClass} ${colorClass}`} />
      case 'swap_request':
      case 'swap_accepted':
      case 'swap_rejected': return <RefreshCw className={`${baseClass} ${colorClass}`} />
      case 'report_submitted':
      case 'report_reviewed': return <AlertCircle className={`${baseClass} ${colorClass}`} />
    ...
    return (
      <div
        ...
        className={`group relative flex items-start gap-3 p-4 transition-colors hover:bg-muted/50 ${
          isUnread ? 'bg-background cursor-pointer' : 'bg-transparent cursor-pointer opacity-80 hover:opacity-100'
        } ${n.action_url ? 'hover:shadow-sm' : ''}`}
      >
        <div className={`mt-0.5 rounded-full p-1.5 ${isUnread ? 'bg-primary/10' : 'bg-muted'}`}>
          {getIcon(n.type, !isUnread)}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <div className={`text-sm leading-tight ${isUnread ? 'font-semibold text-foreground' : 'font-medium text-foreground/80'}`}>
              {n.title}
            </div>
            {isUnread && (
          ...
          <p className={`text-xs leading-relaxed line-clamp-2 ${isUnread ? 'text-muted-foreground' : 'text-muted-foreground/80'}`}>
            {n.message}
          </p>
  ...
  return (
    <div className="relative" ref={panelRef}>
      <Button
        variant="ghost"
        size="icon"
        className={`relative ${unreadCount > 0 ? 'text-foreground' : 'text-muted-foreground'}`}
