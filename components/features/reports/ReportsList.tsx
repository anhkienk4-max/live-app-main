...
export function ReportsList() {
  ...
  const stats = React.useMemo(() => {
    return {
      ...
      totalRevenue: reports.filter(r => r.metrics_confirmed).reduce((sum, r) => sum + r.revenue, 0),
      averageRevenue: reports.filter(r => r.metrics_confirmed).length > 0 ? reports.filter(r => r.metrics_confirmed).reduce((sum, r) => sum + r.revenue, 0) / reports.filter(r => r.metrics_confirmed).length : 0,
              ...
              >
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className={report.metrics_confirmed ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}>{report.metrics_confirmed ? 'Confirmed' : 'Needs review'}</Badge>
                  </div>
