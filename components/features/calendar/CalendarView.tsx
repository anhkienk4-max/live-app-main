...
export function CalendarView() {
  ...
  const filteredShifts = React.useMemo(() => {
    return shifts.filter(shift => {
      ...
      if (filters.support !== 'all' && shift.support_id !== filters.support) return false
      if (filters.technical !== 'all' && shift.technical_id !== filters.technical) return false
  ...
  const clearFilters = () => {
    setFilters({ brand: 'all', platform: 'all', status: 'all', host: 'all', support: 'all', technical: 'all' })
    setSearchTerm('')
  }
        ...
        <div className="space-y-4">
          ...
          {showFilters && (
            <div className="grid grid-cols-3 gap-4 pt-4 border-t">
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Support</label>
                <Select value={filters.support} onValueChange={(value) => setFilters({ ...filters, support: value })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="all">All Support</SelectItem>{users.filter(u => u.operational_roles?.includes('support') || u.department === 'Live Support').map(u => <SelectItem key={u.id} value={u.id}>{u.full_name}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Technical</label>
                <Select value={filters.technical} onValueChange={(value) => setFilters({ ...filters, technical: value })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="all">All Technical</SelectItem>{users.filter(u => u.operational_roles?.includes('technical')).map(u => <SelectItem key={u.id} value={u.id}>{u.full_name}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              {hasActiveFilters && (
