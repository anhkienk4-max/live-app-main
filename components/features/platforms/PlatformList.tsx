...
export function PlatformList() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (success) {
      toast({ title: 'Success', description: 'Platform deleted', variant: 'success' })
      loadPlatforms()
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selected) {
        ...
        toast({ title: 'Success', description: 'Platform updated', variant: 'success' })
      } else {
        await platformService.create(formData)
        toast({ title: 'Success', description: 'Platform created', variant: 'success' })
      }
      loadPlatforms()
      setIsFormOpen(false)
    } catch (error) {
